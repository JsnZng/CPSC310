/**
 *  Created by turbosqel.pl || McFamous ; initialized 02.09.14.
 *
 *  simple xml to json parser to use in browser and node ;
 *
 *  0.2.0 v
 */
...
module xml2json {
    export class XMLParser {
        ...
        private static ParseError(type:number , at:number):any {
            throw new Error('Error:' + ParseError['e'+type]+ ' at char '+at );
        }
    ...
    //////////////////////////////////////////////////
    // -----------------------> XML BUILDER ::
    // TODO :: nice version , only ugly builder atm;

    export class XMLBuilder {

        public static convert(obj:any):string {
            return XMLBuilder.createTag(obj , 0);
        }

        private static createTag(obj:any , depth:number):string {
            var tag:string = '<' + obj['@tag'];
            for(var name in obj){
                if(name.charAt(0) != '@'){
                    tag += ' ' + name + (obj[name] ?'='+obj[name]:'');
                }
            }
            var c:any = obj['@children'];
            if(c){
                tag += '>';
                for(var i:number = 0; i< c.length ; i++){
                    if(c[i].constructor == String){
                        ...
                        tag += XMLBuilder.createTag(c[i],depth+1);
                    }
                }
                tag += '/>';
            } else {
    ...
    export class ParseError{
        public static e101:string = 'tag not closed';
        public static e102:string = 'missing attribute closer';
    }
